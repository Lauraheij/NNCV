...
class DistillationLoss(nn.Module):
    def __init__(self, alpha=0.5, temperature=4.0, ignore_index=255):
        super().__init__()
        self.alpha = alpha          # balance between CE and distillation
    ...
    def forward(self, student_logits, teacher_logits, labels):
        ...
        kl_loss = F.kl_div(student_soft, teacher_soft, reduction='none')
...
def main(args):
    ...
    train_transforms = v2.Compose([
        v2.ToImage(),
        v2.RandomResizedCrop(size=(168, 336), scale=(0.5, 1.0), #orginal:size=(252, 518)
    ...
    val_transforms = v2.Compose([
        v2.ToImage(),
        v2.Resize((168, 336), interpolation=v2.InterpolationMode.BILINEAR),
        v2.ToDtype(torch.float32, scale=True),
        v2.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
])
    ...
    teacher = TeacherModel(n_classes=19, backbone='dinov2_vitb14').to(device)
    teacher.load_state_dict(torch.load("model_DinoV2_DPT.pt", map_location=device))
    teacher.eval()
    ...
    optimizer = AdamW([
        ...
        {'params': model.reassembles.parameters(), 'lr': args.lr * 2},
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, labels) in enumerate(train_dataloader):
            ...
            teacher_images = F.interpolate(images, size=(518, 518), mode='bilinear', align_corners=False)
        ...
        with torch.no_grad():
            ...
            for i, (images, labels) in enumerate(valid_dataloader):
                ...
                teacher_images = F.interpolate(images, size=(518, 518), mode='bilinear', align_corners=False)
                teacher_outputs_val = teacher(teacher_images)
                
                teacher_outputs_val = F.interpolate(teacher_outputs_val, size=outputs.shape[2:], mode='bilinear', align_corners=False)
                
                loss, _, _ = criterion(outputs, teacher_outputs_val, labels)
        ...
        if epoch_miou.item() > best_miou:
            ...
            if current_best_miou_path and os.path.exists(current_best_miou_path):
                try:
