...
class EfficientFusionHead(nn.Module):
    def __init__(self, channels=128, n_classes=19): # Reduced channels
        super().__init__()
        
        # 
        self.fusion = nn.Sequential(
            # Depthwise Separable Convolution instead of standard Conv2d
...
class Model(nn.Module):
    def __init__(self, in_channels=3, n_classes=19, backbone='dinov2_vits14'):
        super().__init__()
        ...
        state_dict = torch.load(
            os.path.join(base_dir, 'dinov2_vits14_pretrain.pth'),
            map_location='cpu'
        )
        ...
        #DPTReassemble instead of FeatureProjection
        self.reassembles = nn.ModuleList([
            DPTReassemble(embed_dim, 64) for _ in range(4)
        ])
    ...
    def forward(self, x):
        ...
        # target is 1/4 of the downsampled input resolution
        target_hw = (H // 4, W // 4)
